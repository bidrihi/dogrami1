...
<main id="main">
    ...
    <div class="container" align="center">
        <div class="col-9">
            <a href="${ pageContext.servletContext.contextPath }/dailylist.do"><h1 style="font-family: 'siu'">일상잡담</h1>
            </a>
        </div>
    </div>
    <br>
    ...
    <div class="container" align="center">
        <div class="col-9">
            <table class="table table-striped" style="text-align: center;">
                ...
                <tbody>
                <c:forEach items="${ requestScope.list }" var="daily" varStatus="status">
                    <tr>
                        <td>${daily.board_no}</td>

                        <c:url var="dbdt" value="/dailydetail.do">
                            <c:param name="board_no" value="${ daily.board_no }"/>
                            <c:param name="page" value="${currentPage}"/>
                        </c:url>
                        <td><a href="${dbdt}">${ daily.board_title }</a> [${replycount[status.index]}]</td>

                        <td>${daily.member.user_nick}</td>

                        <td><fmt:formatDate value="${ daily.board_date }" pattern="yyyy-MM-dd"/></td>

                        <td>${ daily.board_count}</td>
            ...
            <div class="col-12" align="right">
                <c:if test="${!empty sessionScope.loginMember}">
                    <input type="button" onclick="javascript:location.href='dailywrite.do';" class="btn btn-primary"
                           value="글쓰기">
                </c:if>
            </div>


            <form action="dailysearch.do" name="search" method="post">
                <table>
                    <tr>
                        <td>
