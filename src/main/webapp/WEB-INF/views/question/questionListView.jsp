...
<main id="main">
    ...
    <div class="container" align="center">
        <div class="col-9">
             <a href="${ pageContext.servletContext.contextPath }/questionlist.do"><h1 style="font-family: 'siu'">질문답변</h1></a>
        </div>
    </div>
    <br>
    <br>
    <div class="container" align="center">
        <div class="col-9">
            <table class="table table-striped" style="text-align: center;">
                ...
                <tbody>
                <c:forEach items="${ requestScope.list }" var="question" varStatus="status">
                <tr>
                   <td>${question.board_no}</td>
                   
					<c:url var="dbdt" value="/questiondetail.do">
						<c:param name="board_no" value="${ question.board_no }" />
						<c:param name="page" value="${currentPage}"/>
					</c:url>
					<td><a href="${dbdt}">${ question.board_title }</a> [${replycount[status.index]}]</td>
					
					<td>${question.member.user_nick}</td>
					
					<td><fmt:formatDate value="${ question.board_date }" pattern="yyyy-MM-dd" /></td>
					
					<td>${ question.board_count}</td>
		 ...
		 <!-- 로그인한 회원만 게시글 등록(글쓰기) 버튼이 보이게 함 -->
			<div class="col-12" align="right">
			 	<c:if test="${!empty sessionScope.loginMember}">
					<input type="button" onclick="javascript:location.href='questionwrite.do';" class="btn btn-primary" value="글쓰기">
				</c:if>
			</div>
			            
                
            <form action="questionsearch.do" name="search" method="post">
                <table>
                    <tr>
                        <td>
