...
<head>
    ...
    <script type="text/javascript">
        ...
        function deleteBtn(i) {
            document.getElementById("ref_content_frm_" + i).action = "preplyDelete3.do";
        ...
        function clickLikeBtn() {
            ...
            //본인이 작성한 게시글이면
            if ("${sessionScope.loginMember.user_id}" == "${requestScope.placeBoard.user_id}") {
            ...
            //로그인했으면
                ...
                location.href = "placelikeCountUp.do?board_no=${placeBoard.board_no}&user_id=${loginMember.user_id}";
            }
        }

        function clickUnLikeBtn() {
            location.href = "placelikeCountDown.do?board_no=${placeBoard.board_no}&user_id=${loginMember.user_id}";
        }
...
<main id="main">
        ...
        <div class="col-9">
            <table class="table table-bordered" style="text-align: center; table-layout: fixed">
                <tr>
                    ...
                    <td style="width: 20%;">${placeBoard.member.user_nick}</td>

                    <th style="width: 20%;">작성날짜</th>
                    <td><fmt:formatDate value="${placeBoard.board_date}" type="date"
                                        pattern="yyyy-MM-dd HH:mm:ss"/></td>
                </tr>
                <tr>
                    <th>분류</th>
                    <td colspan="3">${placeBoard.category}</td>
                </tr>
                <tr>
                    <th style="vertical-align: middle">지역</th>
                    <td style="vertical-align: middle">${placeBoard.location}</td>
                    <th style="vertical-align: middle">별점</th>
                    <td>
                        <div class="star">
                            <c:if test="${placeBoard.star_point eq 0}">&nbsp;</c:if>
                            <c:if test="${placeBoard.star_point eq 1}">★</c:if>
                            <c:if test="${placeBoard.star_point eq 2}">★★</c:if>
                            <c:if test="${placeBoard.star_point eq 3}">★★★</c:if>
                            <c:if test="${placeBoard.star_point eq 4}">★★★★</c:if>
                            <c:if test="${placeBoard.star_point eq 5}">★★★★★</c:if>
                        </div>
                    </td>
                </tr>
                <tr>
                    ...
                    <td colspan="3">
                        <!-- 사진이 있으면 보여줌 -->
                        <c:if test="${!empty placeBoard.new_image}">
                ...
                <tr>
                    ...
                    <td colspan="3" style="width: 85%; word-break: break-all; word-wrap: break-word;">
                        ${fn:replace(placeBoard.board_content, crcn, br)}
                    </td>
                </tr>
            </table>
            ...
            <div style="text-align: left; float: left">
                ...
                <p align="center" style="font-size: 10pt;">${placeBoard.like_count}</p>

            </div>
            <br>
            <div class="col-12" align="right">
                <!-- 로그인 했고 유저아이디랑 로그인멤버 유저아이디랑 같을시 수정 삭제기능 -->
                <c:if test="${ !empty sessionScope.loginMember and placeBoard.user_id eq loginMember.user_id}">
                    <c:url var="pupdate" value="/pupdate3.do">
                        <c:param name="board_no" value="${placeBoard.board_no}"/>
                        <c:param name="page" value="${currentPage}"/>
                    </c:url>
                    <a href="${pupdate}" class="btn btn-primary">수정</a> &nbsp;
                ...
                <!-- 관리자일시 삭제처리만 가능하게끔 -->
                <c:if test="${loginMember.user_admin eq 'Y' and loginMember.user_id ne placeBoard.user_id }">
                    <c:url var="pdelete" value="/pdelete3.do">
                        <c:param name="board_no" value="${placeBoard.board_no}"/>
                        <c:param name="page" value="${currentPage}"/>
                    </c:url>
                ...
                <a href="${ pageContext.servletContext.contextPath }/plist3.do?page=${currentPage}"
    ...
    <div class="container" align="center">
        <div class="col-9" style="text-align: left">
            <div class="comments">
                ...
                <c:forEach items="${placeReply}" var="reply" varStatus="status">
                    <div class="comment d-flex mb-4">
                        <!-- 댓글 업데이트 form -->
                        <form action="preplyUpdate3.do" method="post" id="ref_content_frm_${reply.ref_no}">
                            <div class="flex-grow-1 ms-2 ms-sm-3">
                                <div class="comment-meta d-flex align-items-baseline">
                                    ...
                                    <c:if test="${ !empty loginMember and loginMember.user_id eq reply.ref_writer }">
                                        ...
                                        <c:url var="pdelete" value="preplyDelete3.do">
                                            <c:param name="page" value="${currentPage }"/>
                                        ...
                                        <span class="text-muted"><a href="#"
                                                                    onclick="javascript:location.href='${pdelete}'"
    ...
    <form action="prepinsert3.do" method="post">
